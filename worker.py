...
class Worker(QThread):
    ...
    def _restore(self):
        ...
        common_jobs = [
            {
                ...
                "dest": r"C:\SEPR\Drivers",
                "type": "folder",
                "progress": 0,  # 드라이버 복사는 별도 진행률 계산 안함
            },
            {
                "name": "시작 메뉴 레이아웃 복원",
                "source": start_menu_source_file,
        ...
        for job in restore_jobs:
            ...
            # 원본 경로가 존재하지 않으면 경고 로그를 남기고 작업을 건너뜀
            if not os.path.exists(source_path):
                # 드라이버 복사 작업은 건너뛰어도 진행률에 영향을 주지 않으므로 로그만 남김
            ...
            # progress 값이 0보다 큰 경우에만 진행률 업데이트
            if job["progress"] > 0:
                self._update_progress(job["progress"])
